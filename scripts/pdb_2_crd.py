...
def split_chain(fhandle):
    ...
    carb_residues = ['AFL','ALL', 'BMA', 'BGC', 'BOG', 'FCA', 'FCB', 'FMF',' FUC', 'FUL', 'G4S', 'GAL', 'GLA', 'GLB', 'GLC', 'GLS', 'GSA', 'LAK', 'LAT', 'MAF', 'MAL', 'NAG', 'NAN', 'NGA', 'SIA', 'SLB']
...
def split_chain_hetatm(fhandle):
    ...
    chain_ids =  None
    records = ('HETATM', 'TER')
    carb_residues = ['NAG', 'BMA', 'MAN', 'GAL', 'FUL', 'FUC', 'AFL', 'RIB',' GLC', 'ALT', 'ALL', 'GUL', 'BGC', 'IDO', 'TAL', 'XYL', 'RHM', 'XYL', 'SIA', 'HEM']
    chain_ids = []
    with open(fhandle, 'r') as infile:
        for line in infile:
            if line.startswith(records):
        ...
        for chain_id in sorted(chain_data.keys()):
            ...
            if carb_present:
                ...
                with open(file_pdb, 'r') as infile:
                    ...
                    modified_lines = [line.replace('HETATM', 'ATOM  ') for line in lines]
...
def write_pdb_2_crd (chain_ids):
    with open('pdb_2_crd.inp', 'w') as outfile:
        ...
        outfile.write (f"* GENERATED BY CHARMM-GUI (HTTP://WWW.CHARMM-GUI.ORG) V3.7 ON JAN, 23. 2024\n")
        outfile.write (f"* READ PDB, MANIPULATE STRUCTURE IF NEEDED, AND GENERATE TOPOLOGY FILE\n")
        outfile.write (f"*  DATE:     1/24/24     12: 4:41      CREATED BY USER: mhammel\n")
        outfile.write (f"*\n")
