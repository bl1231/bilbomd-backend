...
sub setup {
        ...
        $Toppardir = "/home/mhammel/bilbomd_development/bilbomdgly/charmm-gui/topparmichal/topparmichal.str"; # CHARMM tompology  files
	...
	if ($total  != 6) { 
		...
		print "\n";
		print "\n";
		print "psf  is the CHARMM.psf parameter file, file.crd need to be in working directory too.(use .psf extension.) \n ";
	...
	&cleaning
		...
		$file =~ tr/[A-Z]/[a-z]/;
                $cmd = "cp -f  $psf $file.psf "; system $cmd;
                $cmd = "cp -f  $crd $file.crd "; system $cmd;
...
sub input_for_dynamics_and_foxs {
		...
		$dat_dirty = $ARGV[1];
                ...
                unless (-e $dir.'/'.$dat_dirty)    {
                        print "!!!!!!! Experimental DATA file $dat_dirty do not exist, start  aggain !!!!!!!\n ";
			&cleaning;
		}
		...
		if ($StartNumrg) {
			unless ($StartNumrg !~/\D/ && $StartNumrg =~/^[1-9]/ && $StartNumrg < 100) { 
				print "Rg min  need to be in the range 10 - 100 \n";
				&cleaning;
			}
		}
...
sub foxs_from_new_dcd {
		...
		open (FOXS, ">$dir/$file.foxs_rg.out") || die  "Could not open file $file.foxs_rg.out : $!\n";        
		...
		until ($countend == $countstart) {	
					...
					if  (  -e  $dir.'/'.$file.$RG.'_'.$r.'.rst' && -e $dir.'/'.$file.$RG.$r.'.end') { 
						...
						open(DCD, $dir.'/' .$file. '' .$RG. '_' .$r. '.dcd') || die  "Could not open $dcd  file : $!\n";
						open(RST, $dir.'/' .$file. '' .$RG. '_' .$r. '.rst') || die  "Could not open $rst  file : $!\n";
						close (RST);
						close (DCD);
...
*
						...
						system $cmd;
						print "calculating FOXS  fits for Rg = $RG   ,  run = $r    \n";	
...
sub bilbomd_done {
        ...
        system $cmd;
	$cmd = "rm -f  $dir/*.end* $dir/*.start $dir/*0.inp $dir/*1.inp $dir/*2.inp $dir/*3.inp $dir/*4.inp $dir/*5.inp $dir/*6.inp $dir/*7.inp $dir/*8.inp $dir/*9.inp ";
        system $cmd;
	$cmd = "rm -f  $dir/*1.out $dir/*2.out $dir/*3.out $dir/*4.out $dir/*5.out $dir/*6.out $dir/*7.out $dir/*8.out $dir/*9.out  ";
        system $cmd;
	
        # zipping results
        #print "zipping results... \n";

	$cmd = " cd $dir; zip $file.zip $dat cluster_representatives.txt  multi_state_model* ensembles_size*  const.inp *.pdb ";
	system $cmd;
...
sub cleaning {
	print" \n";
	print" !!!!!!!!!!  Abnormal Termination !!!!!!!! ...... check status.txt \n";
	print" cleaning ..........................\n";
	$cmd = "rm -f  $dir/fit/*.pdb ";
        system $cmd;
	$cmd = "rm -f  $dir/*.end* $dir/*.start $dir/*0.inp $dir/*1.inp $dir/*2.inp $dir/*3.inp $dir/*4.inp $dir/*5.inp $dir/*6.inp $dir/*7.inp $dir/*8.inp $dir/*9.inp ";
	system $cmd; 
        $cmd2 = "rm -f  $dir/*1.out $dir/*2.out $dir/*3.out $dir/*4.out $dir/*5.out $dir/*6.out $dir/*7.out $dir/*8.out $dir/*9.out ";
	system $cmd;
	exit;
}
