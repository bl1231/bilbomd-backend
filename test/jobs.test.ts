...
describe('POST /api/v1/jobs', () => {
  ...
  test('should return error if too many files uploaded to a single field', async () => {
    expect.assertions(2)
    const token = generateAccessToken()
    const res = await request(server)
      .post('/api/v1/jobs')
      .set('Authorization', `Bearer ${token}`)
      .field('title', 'Too many files test')
      ...
      .field('bilbomd_mode', 'pdb')
    expect(res.statusCode).toBe(400)
    expect(res.body.message).toMatch(/File upload error/i)
  })
  ...
  test('should return success if new BilboMD CRD job created', async () => {
    expect.assertions(2)
    const token = generateAccessToken()
    const res = await request(server)
      .post('/api/v1/jobs')
      .set('Authorization', `Bearer ${token}`)
      .field('title', 'BilboMD Test Job')
      .attach('crd_file', `${__dirname}/data/crd/pro_dna.crd`)
      .attach('psf_file', `${__dirname}/data/crd/pro_dna.psf`)
      .attach('inp_file', `${__dirname}/data/crd/const.inp`)
      .attach('dat_file', `${__dirname}/data/crd/saxs-data.dat`)
      .field('rg', 35)
      .field('rg_min', 30)
      .field('rg_max', 40)
      .field('num_conf', 1)
    ...
    // console.log('res----->', res.body)
    expect(res.statusCode).toBe(200)
    expect(res.body.message).toBe('New crd_psf Job successfully created')
  })
